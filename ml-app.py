...
#################################### The missingness pattern #############
    st.text("##########################################################################################")
    st.header("Visualization of Missingness Pattern")
    st.text("##########################################################################################")
    st.sidebar.text("##################################")
    st.sidebar.header("Visualization of Missingness Pattern")

    Miss_Pattern=data.copy()
    Miss_Pattern[~np.isnan(Miss_Pattern)]=1.0
    Miss_Pattern[np.isnan(Miss_Pattern)]=0.0
    clustering_methods =[
    ...
    clustering_methods = st.sidebar.selectbox(
        label="Choose...", options=clustering_methods, key=1
    )
    
    if clustering_methods == "t-SNE-2D" or clustering_methods == "t-SNE-3D" :
        n_iter = st.sidebar.slider('n_iter', 50, 1000, 300,key=2)
        perplexity = st.sidebar.slider('perplexity',5, 50, 40,key=2)
        n_cluster = st.sidebar.slider('n_cluster', 2, 10, 2,key=2)
    if clustering_methods == "PCA-2D" or clustering_methods == "PCA-3D" :
        n_cluster = st.sidebar.slider('n_cluster', 2, 10, 2,key=2)

    btn0 = st.sidebar.button("Run-part-0")
    ...
    if btn0:
        ...
        if clustering_methods == "PCA-2D":
            ...
            plt.scatter(PCA_components[0], PCA_components[1], c=labelss,cmap='gist_rainbow_r',s =s, alpha=1)
            plt.xlabel('PCA 1')
            plt.ylabel('PCA 2')
            ...
            ax.scatter(PCA_components[0], PCA_components[1],PCA_components[2], c=labelss,cmap='gist_rainbow_r', alpha=1,s =s)
            ax.set_xlabel('PCA 1')
            ax.set_ylabel('PCA 2')
            ax.set_zlabel('PCA 3')
            ...
            plt.scatter(tsne_results[0], tsne_results[1], c=labelss,cmap='gist_rainbow_r', alpha=1,s=s)
